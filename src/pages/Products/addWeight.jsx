...
const AddWeight = () => {
  ...
  const getRams = async () => {
    try {
      ...
      context.alertBox?.("error", "Failed to fetch Weights");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!ramValue.trim()) {
      context.alertBox?.("error", "Weight name is required");
      return;
    }
    ...
    try {
      if (editId) {
        // Edit existing RAM
        const res = await editData(`/api/product/updateProductWeight/${editId}`, {
          name: ramValue,
        });

        if (res?.success) {
          context.alertBox?.("success", "Weight updated successfully");
        } else {
          context.alertBox?.("error", res.message || "Failed to update Weight");
        }
      } else {
        // Create new RAM
        const res = await postData(`/api/product/productWeight/create`, {
          name: ramValue,
        });

        if (res?.success) {
          context.alertBox?.("success", "Weight  added successfully");
        } else {
      ...
      context.alertBox?.("error", "Server error while submitting Weight ");
    } finally {
      setLoading(false);
    }
  };
  ...
  const deleteRam = (id) => {
    deleteData(`/api/product/productWeight/${id}`).then((res) => {
      getRams();
      context.alertBox("success", "Weight deleted");
    });
  };
  ...
  return (
    <>
      <div className="bg-white shadow-md rounded-xl mx-5 mt-6 px-6 py-4 border border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <h2 className="text-xl font-bold text-gray-800">Add Product Weights</h2>
      </div>
          ...
          <table className="w-full text-sm text-left text-gray-700 border-collapse">
            <thead className="text-xs text-gray-700 uppercase bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 w-[60%]">Product Weight </th>
